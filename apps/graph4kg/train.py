...
def main():
    ...
    train_loader, valid_loader, test_loader = create_dataloaders(
        ...
        shared_ent_path=sgpu_model.shared_ent_path
        if args.mix_cpu_gpu else None)
    ...
    for epoch in range(args.num_epoch):
        ...
        for indexes, prefetch_embeddings, mode in train_loader:
            ...
            ts = time.time()
            h_emb, r_emb, t_emb, neg_emb, mask = sgpu_model.prepare_inputs(
                h, r, t, all_ents, neg_ents, all_ents_emb, rel_emb, mode, args)
            pos_score = model(h_emb, r_emb, t_emb)

            if mode == 'head':
                neg_score = sgpu_model.get_neg_score(t_emb, r_emb, neg_emb,
                                                     True, mask)
            elif mode == 'tail':
                neg_score = sgpu_model.get_neg_score(h_emb, r_emb, neg_emb,
                                                     False, mask)
            else:
            ...
            if args.use_embedding_regularization:
                reg_loss = sgpu_model.get_regularization(h_emb, r_emb, t_emb,
                                                         neg_emb)
            ...
            if args.mix_cpu_gpu:
                ent_trace, rel_trace = sgpu_model.create_trace(
                    all_ents, all_ents_emb, r, r_emb)
                sgpu_model.step(ent_trace, rel_trace)
            else:
                sgpu_model.step()
    ...
    if args.async_update:
        sgpu_model.finish_async_update()
