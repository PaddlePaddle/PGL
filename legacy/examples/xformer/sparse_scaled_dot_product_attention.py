...
def build_edges(num_nodes, input_mask, max_seqlen):
    ...
    for i in range(2):
        rand_edge = L.floor(
            L.uniform_random(
                min=0, max=1, shape=[num_nodes]) * L.cast(
                    max_seqlen, dtype="float32"))
...
def sparse_scaled_dot_product_attention(q, k, v, input_mask, dropout_rate,
                                        n_head, d_key, d_value):
    ...
    msg = graph_wrapper.send(
        send_q_k_spmm, nfeat_list=[("k", k), ("v", v), ("q", q)])
