...
__all__ = [
    'copy_send', 'weighted_copy_send', 'mean_recv', 'sum_recv', 'max_recv',
    'lstm_recv', 'graphsage_sum', 'graphsage_mean', 'pinsage_mean',
    'pinsage_sum', 'softmax_agg', 'msg_norm'
]
...
def graphsage_sum(gw, feature, hidden_size, act, initializer, learning_rate,
                  name):
    """doc"""
...
def graphsage_mean(gw, feature, hidden_size, act, initializer, learning_rate,
                   name):
    """doc"""
    msg = gw.send(copy_send, nfeat_list=[("h", feature)])
...
def pinsage_mean(gw, feature, hidden_size, act, initializer, learning_rate,
                 name):
    """doc"""
    msg = gw.send(
        weighted_copy_send, nfeat_list=[("h", feature)],
        efeat_list=["weight"])
    neigh_feature = gw.recv(msg, mean_recv)
    self_feature = feature
    self_feature = L.fc(self_feature,
...
def pinsage_sum(gw, feature, hidden_size, act, initializer, learning_rate,
                name):
    """doc"""
    msg = gw.send(
        weighted_copy_send, nfeat_list=[("h", feature)],
