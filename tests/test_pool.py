...
class PoolTest(unittest.TestCase):
    ...
    def test_globalattention(self):
        data1 = paddle.to_tensor(np.array([[1, 2, 3], [3, 2, 1], [4, 5, 6]], dtype="float32"))
        data2 = paddle.to_tensor(np.array([[1, 2, 3], [3, 2, 1]], dtype="float32"))
        data3 = paddle.to_tensor(np.array([[1, 2, 3]], dtype="float32"))
        data = paddle.concat([data1, data2, data3], axis=0)
        common_edges = [(0, 0)]
        graph = pgl.Graph.disjoint([pgl.Graph(num_nodes=i, edges=common_edges) for i in [3, 2, 1]], 
                                   merged_graph_index=False).tensor()
        gpool = GlobalAttention(paddle.nn.Linear(3, 1))
        output = gpool(graph, data)
        data1 = paddle.to_tensor(np.array([[4, 5, 6], [3, 2, 1], [1, 2, 3]], dtype="float32"))
        data2 = paddle.to_tensor(np.array([[3, 2, 1], [1, 2, 3]], dtype="float32"))
        data3 = paddle.to_tensor(np.array([[1, 2, 3]], dtype="float32"))
        data = paddle.concat([data1, data2, data3], axis=0)
        output_mix = gpool(graph, data)
        # permutation invariance test
        np.testing.assert_almost_equal(output_mix.numpy().reshape(-1,), output.numpy().reshape(-1,), decimal=5,)
    ...
    def test_sag_pool(self):
        ...
        graph_node_index = paddle.scatter(graph_node_index, 
                                          paddle.arange(1, batch_size + 1), num_nodes.cumsum(0)).astype(paddle.int64)
        g = pgl.Graph(num_nodes=8,
                    edges=edge_index,
                    node_feat={
                        "attr":x
                    },
        ...
        self.assertAlmostEqual(output.shape[0], math.ceil(0.5 * 2) + math.ceil(0.5 * 3) + math.ceil(0.5 * 3))
