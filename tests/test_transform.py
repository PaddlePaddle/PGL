...
class TransformTest(unittest.TestCase):
    ...
    def test_to_dense_batch(self):
        ...
        common_edges = [(0,0)]
        graph = pgl.Graph.disjoint([pgl.Graph(num_nodes=i, edges=common_edges) for i in [3,2,1]], merged_graph_index=False).tensor()
        out, mask = to_dense_batch(data, graph, fill_value=0)
        _out = [[[1,2,3],[3,2,1],[4,5,6]],
                [[1,2,3],[3,2,1],[0,0,0]],
                [[1,2,3],[0,0,0],[0,0,0]]]
        self.assertAlmostEqual(out.numpy().tolist(), _out)
        _mask =  [[False, False, False],
                [False, False, True],
                [False, True, True]]
