...
class SAGPooling(nn.Layer):
    ...
    Args:
        input_dim: the dimension of node feature
        ratio: the ratio to reserve topk nodes
        GNN: Graph Convolution Operation to generate score for every node
        min_score: if min_score is not None, the operator only remove 
                            the node with value lower than min_score
        nonlinearity: a nonlinear transform on score when min_score is not None
        
        graph: pgl.Graph
        x: input nodes feature
    Return:
        x: reserved node features
        batch: the updated graph_node_id
        g: the updated graph (pgl.Graph)

    """
    def __init__(self, input_dim, ratio=0.5, GNN=GCNConv, min_score=None,
                nonlinearity=paddle.tanh):
    ...
    def forward(self, graph, x):
        ...
        batch = batch[rank]
        edge_index, edge_attr = filter_adj(graph.edges, rank, num_nodes=attn_score.shape[0])
        batch_size = graph.num_graph
        num_nodes = segment_sum(paddle.ones([x.shape[0]]), batch)
        graph_node_index = paddle.scatter(paddle.zeros([batch_size + 1]).astype(paddle.float32),
                                          paddle.arange(1, batch_size + 1),
                                          num_nodes.cumsum(0).astype(paddle.float32)).astype(paddle.int64)
        g = pgl.Graph(num_nodes=x.shape[0],
...
def segment_topk(x, ratio, segment_ids, min_score=None):
    ...
    if min_score is not None:
        scores_max = math.segment_max(x, segment_ids).index_select(segment_ids, 0) - tol
        scores_min = scores_max.clip(max=min_score)
        perm = (x > scores_min).reshape([-1]).nonzero(as_tuple=False).reshape([-1])
    else:
        num_nodes = segment_sum(paddle.ones([x.shape[0]]), segment_ids)
        batch_size, max_num_nodes = int(num_nodes.shape[0]), int(num_nodes.max().item())
        if num_nodes.cumsum(0).shape[0]>1:
            cum_num_nodes = paddle.concat([paddle.zeros([1]), num_nodes.cumsum(0)[:-1]], 0)
        else:
        ...
        dense_x = paddle.full([batch_size * max_num_nodes], -1e20).astype(paddle.float32)
        ...
        if isinstance(ratio, int):
            ...
            k = (ratio * num_nodes.astype(paddle.float32)).ceil().astype(paddle.int64)
        mask = [
            paddle.arange(k[i], dtype=paddle.int64,) + i * max_num_nodes for i  in range(batch_size)
        ]
