...
optimizer = Adam(learning_rate=args.lr,parameters=[{
...
def train(st,end):
    ...
    acc_train = paddle.metric.accuracy(input=output, label=train_labels[st:end], k=1)
...
def validate(st,end):
    ...
    acc_val = paddle.metric.accuracy(input=output, label=valid_labels[st:end], k=1)
...
def test(st,end):
    ...
    acc_test = paddle.metric.accuracy(input=output, label=test_labels[st:end], k=1)
