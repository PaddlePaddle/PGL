...
class FAGCN(nn.Layer):
    ...
    def __init__(
        self, input_size, hidden_size, output_size, drop=0.6, eps=0.2, num_layer=3
    ):
    ...
    def forward(self, graph, feature):
        ...
        feature = paddle.nn.functional.relu(self.t1(feature))
        feature = self.dropout(feature)
...
def main(args):
    ...
    for run in range(args.runs):
        ...
        optim = Adam(
            learning_rate=0.005, parameters=gnn_model.parameters(), weight_decay=0.0005
        )
        ...
        log.info(
            ...
            % (run, type(gnn_model).__name__, cal_test_acc[np.argmin(cal_val_loss)])
        )
