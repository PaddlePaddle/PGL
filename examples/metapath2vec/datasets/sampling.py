...
__all__ = [
    ...
    'metapath_randomwalk_with_walktimes',
]
...
def node2vec_sample(graph, nodes, max_depth, p=1.0, q=1.0):
    ...
    cur_walk_ids = np.arange(0, len(nodes))
    cur_nodes = np.array(nodes, dtype="uint64")
    prev_nodes = np.array([-1] * len(nodes), dtype="uint64")
    prev_succs = np.array([[]] * len(nodes), dtype="uint64")
    for l in range(max_depth):
        # select the walks not end
        cur_succs = graph.successor(cur_nodes)
...
def metapath_randomwalk(graph,
    ...
    cur_nodes = np.array(start_nodes, dtype="uint64")
    mp_len = len(edge_types)
...
def node2vec_randomwalk_with_walktimes(
        graph,
        start_nodes,
        metapath,
        walk_length,
    ...
    if q == 1.0 and p == 1.0:
        warnings.warn(
            "node2vec with p==1.0 and q==1.0 with degenerate into deepwalk")
        return metapath_randomwalk_with_walktimes(
            graph=graph,
    ...
    if len(set(edge_types)) > 1:
        raise ValueError("node2vec must have single metapath")

    n1, n2 = edge_types[0].split("2")
    if n1 != n2:
        raise ValueError("node2vec must have same node types between edges.")
    ...
    neighbors = graph.sample_successor(
        np.array(
            start_nodes, dtype="uint64"),
        max_degree=max(walk_times, MAX_NEIGHBORS),
        edge_type=edge_types[0])

    prev_succs = []
    for neigh, walk_id in zip(neighbors, start_nodes):
        for node_id in neigh[:walk_times]:
            walk.append([walk_id, node_id])
            cur_nodes.append(node_id)
            prev_nodes.append(walk_id)
            prev_succs.append(neigh)
    ...
    cur_walk_ids = np.arange(0, len(walk))
    cur_nodes = np.array(cur_nodes, dtype="uint64")
    prev_nodes = np.array(prev_nodes, dtype="uint64")
    prev_succs = np.array(prev_succs, dtype="object")
    #  if np.random.random() - 0.02 < 0:
    #      sys.stderr.write("length of walks %s\n" % (len(walk)))
    ...
    for i in range(1, walk_length - 1):
        ...
        num_nodes = cur_nodes.shape[0]
        nxt_nodes = np.zeros(num_nodes, dtype="uint64")

        for idx, (
                succ, prev_succ, walk_id, prev_node
        ) in enumerate(zip(cur_succs, prev_succs, cur_walk_ids, prev_nodes)):

            succ = np.array(succ, dtype="int64")
            prev_succ = np.array(prev_succ, dtype="int64")
            sampled_succ = graph_kernel.node2vec_sample(succ, prev_succ,
                                                        prev_node, p, q)
            if sampled_succ is None:
                sampled_succ = succ[-1]
        ...
        prev_nodes, prev_succs = cur_nodes, cur_succs
        cur_nodes = nxt_nodes.astype("uint64")
    return walk
