...
fp16_list = re.compile(r'distill_hidden|distill_attn|encoder_layer.*w_0|encoder_layer.*b_0|student_encoder_layer.*w_0|student_encoder_layer.*b_0')
...
def cast_fp32_to_fp16(exe, main_program):
    ...
    for param in main_program.global_block().all_parameters():
        if not param.name.endswith(".master"):
            ...
            if fp16_list.match(param.name):
                param_t.set(np.float16(data).view(np.uint16), exe.place)
                log.debug('cast weight [%s] to fp16' % param.name)
            #load fp32
