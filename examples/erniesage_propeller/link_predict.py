...
class ERNIESageLinkPredictModel(propeller.train.Model):
    ...
    def forward(self, features):
        ...
        graph_wrapper = BatchGraphWrapper(num_nodes, num_edges, edges, node_feat)

        encoder = Encoder.factory(self.hparam)
        outputs = encoder([graph_wrapper], [user_index, pos_item_index, neg_item_index])
...
def predict(config):
    ...
    for user_feat, user_real_index in est.predict(predict_ds, ckpt=-1):
