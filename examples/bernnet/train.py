...
def main(args):
    ...
    for run in tqdm.tqdm(range(args.runs)):
        ...
        gnn_model = BernNet(
            ...
            dprate=args.dprate)
        optim = []
        optim1 = Adam(learning_rate=args.lr,parameters=gnn_model.linear_1.parameters(),weight_decay=args.weight_decay)
        optim2 = Adam(learning_rate=args.lr,parameters=gnn_model.linear_2.parameters(),weight_decay=args.weight_decay)
        optim3 = Adam(learning_rate=args.prop_lr,parameters=gnn_model.prop.parameters(),weight_decay=args.prop_wd)
        optim.append(optim1)
        optim.append(optim2)
        optim.append(optim3)
        ...
        for epoch in range(args.epoch):
            ...
            test_loss, tmp_test_acc = eval(test_index, test_label, gnn_model, graph, criterion)
    ...
    log.info("Dataset: %s Best Test Accuracy: %f ± %f " % (args.dataset, np.mean(best_test)*100, np.std(best_test)*100))

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='BernNet')
    parser.add_argument('--seed', type=int, default=42, help='seed.')
    parser.add_argument("--dataset", type=str, default="cora", help="dataset (cora, pubmed)")
    parser.add_argument("--epoch", type=int, default=1000, help="Epoch")
    parser.add_argument("--runs", type=int, default=10, help="runs, fixed 10 random splits")
    parser.add_argument("--net", type=str, default='BernNet', help="the model")
