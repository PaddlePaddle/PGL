...
class GATConv(nn.Layer):
    ...
    def __init__(self,
                 ...
                 num_heads=1,
                 concat=True,
                 activation=None, use_edge=False, dis_type="square"):
    ...
    def _send_attention(self, src_feat, dst_feat, edge_feat):
        rel_coors = src_feat['pos_src'] - dst_feat['pos_dst']
        rel_dist = (rel_coors**2).sum(axis=-1, keepdim=True)
        pos_weight = self.pos_mlp(rel_dist)
        if "edge_feat" in edge_feat:
            edge_feat = self.edge_mlp(edge_feat["edge_feat"])
            alpha = src_feat["src"] + dst_feat["dst"] + edge_feat + pos_weight
        else:
            alpha = src_feat["src"] + dst_feat["dst"] + pos_weight
    ...
    def forward(self, graph, feature, pos, edge_feat=None):
        """
        ...
        if edge_feat is not None:
            msg = graph.send(
                ...
                src_feat={"src": attn_src,
                          "pos_src": pos,
                          "h": feature},
                dst_feat={"dst": attn_dst, "pos_dst": pos},
