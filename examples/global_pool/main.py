...
def main(args):
    ...
    for fold in range(10):
        train_ds, test_ds = fold10_split(
        ds, fold_idx=fold, seed=args.seed)
        ...
        boundaries = [
            i
            for i in range(50 * epoch_step, args.epochs * epoch_step, epoch_step *
                        50)
        ]
        values = [args.lr * 0.5 ** i for i in range(0, len(boundaries) + 1)]
        scheduler = paddle.optimizer.lr.PiecewiseDecay(
            boundaries=boundaries, values=values, verbose=False)
        optim = Adam(learning_rate=scheduler, parameters=model.parameters(), weight_decay=args.weight_decay)
    ...
    with open("outputs/" + args.dataset_name + "-" + args.pool_type + ".log", "a") as f:
        f.write(str(args) + "\n")
        f.write(str(res) + "\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="sum",
        choices=["sum", "mean", "max", "meanmax", "GlobalAttention", "Set2Set", "GMT"])
