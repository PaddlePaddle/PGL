...
def save_pred_result(output_dir, mode, pred_dict):
    save_file = os.path.join(output_dir, "%s_mae.txt" % mode)
    log.info("saving %s result to txt ..." % mode)
    with open(save_file, 'w') as f:
        for pred, label, s in zip(pred_dict['y_pred'], pred_dict['y_true'],
                                  pred_dict['smiles']):
            f.write("%s\t%s\t%s\n" % (pred, label, s))
...
def main(config):
    ...
    if config.split_mode == "cross1":
        ...
        left_valid_ds = DS.Subset(
            ds, split_idx['valid_left_1percent'], mode='valid')
    ...
    train_and_eval(model, config, train_loader, valids, test_loader, optimizer,
                   scheduler)
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if dist.get_rank() == 0 and config.to_valid_step < epoch:
            ...
            if valid_mae < topk_best[topk_num - 1][1]:
                ...
                tmp_output_dir = os.path.join(config.output_dir, "%03d" %
                                              to_rm[0])
