...
#            edge_index = np.unique(edge_index, axis=0)
        ...
        if not osp.exists(edge_path):
            ...
            full_graph = Graph(edge_index, num_nodes=N, edge_feat={'edge_type': edge_types})
            full_graph.adj_dst_index
            full_graph.dump(edge_path)
            log.info(f'Done! finish full_edge [{time.perf_counter() - t:.2f}s]')
        ...
        self.graph._edge_feat['edge_type'] = self.graph._edge_feat['edge_type'].astype('int32')
...
def neighbor_sample(graph, nodes, samples):
    ...
    for max_deg in samples:
        ...
        if max_deg == -1:
            ...
            pred_nodes, pred_eids = graph.sample_predecessor(start_nodes, max_degree=max_deg, return_eids=True)
