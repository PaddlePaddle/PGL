...
def eval_test(parser, program, model, test_exe, graph, y_true, split_idx):
    ...
    for subgraph in random_partition(
            num_clusters=5, graph=graph, shuffle=False):
...
def train_loop(parser,
    ...
    for epoch_id in tqdm(range(parser.epochs)):
        for subgraph in random_partition(
                num_clusters=9, graph=graph, shuffle=True):
            #start training  
            if parser.use_label_e:
                feed_dict = model.gw.to_feed(subgraph)
                ...
                train_idx_temp = set(split_idx['train']) & set(
                    subgraph.node_feat["nid"])
                train_idx_temp = subgraph.reindex_from_parrent_nodes(
                    list(train_idx_temp))
                feed_dict['label'] = subgraph.node_feat["label"]
